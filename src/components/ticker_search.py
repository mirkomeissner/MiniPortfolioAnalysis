...
def get_ref_options(table_name, column_name="code"):
    """Holt die gültigen Optionen (Kürzel) aus den REF-Tabellen in Supabase."""
    try:
...
def get_country_mapping():
    """Holt das Mapping Land -> Region aus der Datenbank."""
    try:
...
def map_yahoo_to_ref(yahoo_sector):
    """Übersetzt Yahoo Sektoren in GICS Codes (2-stellig)."""
    mapping = {
...
def map_yahoo_to_instrument_type(quote_type, symbol_name=""):
    """Mapping von Yahoo quoteType auf deine Instrumenten-Kürzel."""
    mapping = {
...
def map_yahoo_to_asset_class(quote_type, symbol_name=""):
    """Mapping auf Asset Klassen: LIQ, BON, EQU, ALT."""
...
def ticker_search_view():
    ...
    st.write("Suchen Sie per ISIN nach Wertpapieren und passen Sie die Stammdaten an.")

    # REF-Daten initialisieren (Session State Cache)
            ...
            try:
                ...
                if not search_results:
                    ...
                    for res in search_results:
                        ...
                        # Daten für Zeile zusammenstellen (In deiner Wunsch-Reihenfolge)
                        row = {
                            "Ticker": symbol,
                            "Name": name,
