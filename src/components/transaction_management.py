...
def render_transaction_form():
    """Renders the input form for a new transaction."""
    ...
    with st.form("new_transaction_form"):
        ...
        # Reference dropdowns with "Code (Label)" format
        account = col1.selectbox("Account", st.session_state.get('opt_accounts', []))
        asset = col2.selectbox("Asset (ISIN)", st.session_state.get('opt_assets', []))
        
        trans_type = col1.selectbox("Transaction Type", st.session_state.get('opt_trans_types', []))
        date = col2.date_input("Date", value=datetime.now())
        ...
        if st.form_submit_button("Save Transaction", type="primary"):
            # Parsing the codes from the selected strings
            payload = {
