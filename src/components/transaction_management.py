...
def render_import_preview_screen():
    """
    Comprehensive Import Screen featuring:
    1. Persistent Mapping (Account-based)
    2. Advanced Query Filtering (AND/OR logic)
    3. Dry-Run Validation with Auto-Deselection of faulty rows
    4. Auto-Copy EUR Logic for domestic transactions
    """
    ...
    # Use session state as the single source of truth for the data editor
    ...
    # Fetch saved mapping configuration for this account/user
    saved_config = get_import_settings(user, acc_code)
    if saved_config:
        st.success(f"✅ Mapping for account '{acc_code}' loaded.")
    
    # Helper to determine default column index based on saved config or keyword search
    ...
    with st.expander("🛠 Advanced Query Builder", expanded=False):
        ...
        for i, rule in enumerate(st.session_state["import_filter_rules"]):
            ...
            col_name = r1.selectbox(f"Column {i}", csv_columns, key=f"fcol_{i}")
    ...
    # Data Editor for manual selection
    ...
    for i, csv_val in enumerate(distinct_csv_types):
        ...
        type_mapping[csv_val] = target_col.selectbox(f"CSV Value: '{csv_val}'", options=db_trans_types, 
    ...
    if st.button("🚀 Start Import", type="primary", use_container_width=True):
        # Extract rows currently selected in the UI
        ...
        # If errors were found, update the state and block the import
        if invalid_indices:
            # Deselect faulty rows in the session state
            st.session_state["imported_df"].loc[invalid_indices, "import_row"] = False
            msg_container.warning(f"⚠️ {len(invalid_indices)} transactions contain data errors (e.g., invalid dates or numbers). They have been deselected for review.")
            if st.button("OK - Refresh Selection"):
                st.rerun()
            return # STOPS THE PROCESS HERE

        # --- PHASE B: ACTUAL IMPORT (Only reached if Dry-Run succeeds) ---
        success_count = 0
        ...
        for i, (idx, row) in enumerate(current_selection.iterrows()):
            try:
                ...
                st.error(f"Database error in row {idx}: {e}")
            progress_bar.progress((i + 1) / len(current_selection))

        # Persist Mapping Config for future use
        current_config = {
